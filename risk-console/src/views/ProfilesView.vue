    ...
    <article class="panel">
      ...
      <PageState v-else :loading="resource.loading.value" :error="resource.error.value" @retry="resource.load">
        <template v-if="resource.data.value">
          ...
          <div class="split section-gap"><article class="panel"><div class="panel-header"><h3>实时特征</h3><span>{{ resource.data.value.accountNo }}</span></div><div class="key-values"><div v-for="(value,key) in resource.data.value.onlineFeatures" :key="key"><label>{{ key }}</label><b>{{ value }}</b></div></div></article><article class="panel"><div class="panel-header"><h3>标签口径</h3><span>{{ resource.data.value.definitions.length }} 项</span></div><div class="detail-stack"><div v-for="tag in resource.data.value.definitions" :key="tag.tag_code" class="panel tag-definition"><b>{{ tag.tag_name }}</b><p>{{ tag.definition_text }} · v{{ tag.version_no }}</p></div></div></article></div>
        </template>
      </PageState>
    </article>
  </div>
